...
const envConfig = parseEnv({
  BIDS_PATH: {
    type: 'string',
    default: path.resolve('data', 'cca-bids-sepolia.json'),
  },
  PHASE1_SNAPSHOT_PATH: { type: 'string', default: '' },
  PLAYER_LINKS_PATH: { type: 'string', default: '' },
  OUTPUT_PATH: {
    type: 'string',
    default: path.resolve('data', 'bogo-claims.json'),
  },
  BOGO_POOL_CAP: { type: 'bigint', default: 0n, min: 0n },
});

const bidsPath = process.argv[2] || envConfig.BIDS_PATH;
const snapshotPath = process.argv[3] || envConfig.PHASE1_SNAPSHOT_PATH;
const linksPath = process.argv[4] || envConfig.PLAYER_LINKS_PATH;
const outputPath = process.argv[5] || envConfig.OUTPUT_PATH;

if (!bidsPath) {
  throw new Error('BIDS_PATH is required (arg 1 or env)');
}
...
if (links) {
  ...
  for (const entry of entries) {
    ...
    const address = normalizeAddress(
      entry.evm_address || entry.evmAddress || entry.address || entry.wallet
    );
...
const poolCap = envConfig.BOGO_POOL_CAP;
if (poolCap > 0n) {
