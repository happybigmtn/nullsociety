...
**The blockchain is the ONLY source of truth for game outcomes. The 3D engine must NEVER compute outcomes—it only dramatizes chain-delivered results.**

### How Outcomes Flow

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│  Blockchain     │────▶│  Chain Service  │────▶│  3D Engine      │
│  (RNG Source)   │     │  (Decoder)      │     │  (Visualizer)   │
└─────────────────┘     └─────────────────┘     └─────────────────┘
         │                       │                       │
...
├── engine/
│   ├── GuidedStore.ts        # Zustand transient updates (60 FPS without React re-renders)
│   ├── GuidedRound.ts        # Round state types and phase management
│   └── timeStep.ts           # Fixed timestep helpers for determinism
├── physics/
│   ├── guidedForces.ts       # Attractors, velocity gates, noise modulation
│   ├── noise.ts              # Seeded Perlin/Simplex for organic randomness
│   └── colliders.ts          # Factory helpers for collider shapes
├── scenes/
│   ├── roulette/
│   ├── craps/
│   ├── sicbo/
│   ├── blackjack/
│   ├── baccarat/
│   ├── cardTable/           # Shared card table base
│   ├── war/                 # Dragon Tiger
│   ├── threeCardPoker/      # Three Card Poker
│   ├── casinoHoldem/        # Casino Hold'em / Ultimate Texas Hold'em
│   ├── hilo/                # HiLo card game
│   ├── videoPoker/          # Video Poker machine
│   ├── megaBall/            # Mega Ball lottery
│   └── moneyWheel/          # Dream Catcher / Money Wheel
├── materials/
│   └── MaterialConfig.ts     # PBR materials and shared shader chunks
├── shaders/
│   ├── LightningShader.ts    # Lightning Roulette effect
│   └── SqueezeShader.ts      # Baccarat card squeeze
├── post/
│   └── CasinoPostProcessing.tsx  # Bloom, tone mapping, effects
└── audio/
    ├── AudioManager.ts       # Singleton audio context manager
    ├── CollisionSound.tsx    # Physics-driven impact sounds
    └── AmbientSoundscape.tsx # Background casino atmosphere
```
...
1. LAUNCH: Apply vigorous, random-looking impulse (seeded)
   - No guidance—pure physics chaos
...
- Chips stack with slight random rotation (±5°) for realism
- Stack height = chip count × chip thickness (0.003m)
...
**Texture Atlas**: Single 4096x4096 atlas (13 ranks × 4 suits)
```typescript
...
- No artificial delays—player controls pace
...
**Visual Indicators**:
- 🔵 Blue badge for Banker streak (e.g., "B×5")
- 🔴 Red badge for Player streak (e.g., "P×3")
- Animated glow on "hot" tables (streak ≥ 5)
...
void main() {
  ...
  float bendAngle = uBendStrength * bendAmount * 1.5708;  // Max 90°
  // ... vertex displacement
}
```
...
**Evolution Reference**: First Person Dragon Tiger features dramatic lighting—Dragon and Tiger statues that light up on wins, with synchronized color shifts.
...
Minimal setup—single card pedestal with dramatic lighting:
...
Classic video poker machine simulation—single player, no dealer interaction.
...
│   │  [HOLD] [    ] [HOLD] [HOLD] [    ]  │     │
│   │                                        │     │
│   │  [BET 1] [BET MAX]       [DRAW/DEAL]  │     │
│   ════════════════════════════════════════      │
│                                                  │
│   Credits: 100      Bet: 5      Win: 0          │
└─────────────────────────────────────────────────┘
```
...
- ✅ Can run physics at higher frequency (120Hz)
- ⚠️ Adds complexity for synchronization
- ⚠️ Transfer overhead for transform data
...
- No gameplay impact—purely atmospheric
...
1. Player purchases bingo-style cards (5×5 grid with random numbers 1-51)
2. 20 balls drawn from drum
3. Final ball has a random multiplier (5x, 10x, 25x, 50x, 100x)
4. Payouts based on lines completed × multiplier
...
interface LightningRouletteOutcome extends RouletteOutcome {
  luckyNumbers: Array<{ number: number; multiplier: 50 | 100 | 200 | 300 | 400 | 500 }>;
}
```
...
export function calculateAttractorForce(
  ...
  switch (config.falloffCurve) {
    case 'linear': falloff = 1 - normalizedDist; break;
    case 'quadratic': falloff = 1 - normalizedDist * normalizedDist; break;
    case 'inverse-square':
  ...
  const noiseValue = sampleNoise3D(
    currentPos.x, currentPos.y, currentPos.z + guidance.noiseOffset,
    config.noiseFrequency
  );
  const noiseFactor = 1 + noiseValue * config.noiseAmplitude;

  // Calculate and clamp force
  let forceMag = Math.min(config.baseStrength * falloff * noiseFactor, config.forceClamp);
...
export const MATERIAL_PRESETS: Record<string, MaterialPreset> = {
  felt: {
    roughness: 0.95, metalness: 0.02, envMapIntensity: 0.15,
    color: '#0f3a2e',
  },
  chrome: {
    roughness: 0.08, metalness: 1.0, envMapIntensity: 1.2,
    color: '#b0b8c0',
  },
  polishedWood: {
    roughness: 0.35, metalness: 0.15, envMapIntensity: 0.6,
    color: '#4a2918', clearcoat: 0.5, clearcoatRoughness: 0.25,
  },
  cardFace: {
    roughness: 0.45, metalness: 0.1, envMapIntensity: 0.4,
  },
  cardBack: {
    roughness: 0.6, metalness: 0.05, envMapIntensity: 0.3,
  },
  chip: {
    roughness: 0.4, metalness: 0.2, envMapIntensity: 0.5,
  },
  dice: {
    roughness: 0.28, metalness: 0.18, envMapIntensity: 0.6,
  },
  gold: {
    roughness: 0.2, metalness: 0.9, envMapIntensity: 1.0,
    color: '#ffd700', emissive: '#ffaa00', emissiveIntensity: 0.15,
  },
};
```
...
- **Layout**: 13 ranks × 4 suits = 52 cards
- **Resolution**: 3328×1424 pixels (256×356 per card)
- **UV Calculation**:
  ```typescript
  col = RANKS.indexOf(rank);  // 0-12
  row = SUITS.indexOf(suit);  // 0-3
...
export class CardPeekAnimator {
  ...
  // Phases: lift (30%) → hold (400ms) → snap (20%)
...
export function generateRoundSeed(
  ...
  const gameHashes = {
    roulette: 0x1234, craps: 0x5678, sicbo: 0x9ABC,
    blackjack: 0xDEF0, baccarat: 0x1357, war: 0x2468,
  };
...
```typescript
// Collision sounds
function createDiceImpact(ctx: AudioContext, params: { velocity: number; material: string }): AudioNode;
function createBallBounce(ctx: AudioContext, velocity: number, surface: 'metal' | 'felt'): AudioNode;
function createCardSnap(ctx: AudioContext): AudioNode;

// Continuous loops
function createRollingLoop(ctx: AudioContext, baseFreq: number): { source: AudioBufferSourceNode; gain: GainNode };
function createSpinningLoop(ctx: AudioContext): { source: AudioBufferSourceNode; gain: GainNode };
...
website/src/components/casino/3d/
├── engine/
│   ├── GuidedStore.ts              # Zustand transient updates
│   ├── GuidedRound.ts              # Round state types
│   └── timeStep.ts                 # Fixed timestep helpers
├── physics/
│   ├── guidedForces.ts             # Attractors, velocity gates
│   ├── RoulettePhysics.ts          # Roulette-specific physics
│   ├── CardPhysics.ts              # Bezier deal animations
│   └── noise.ts                    # Seeded Perlin/Simplex
├── materials/
│   └── MaterialConfig.ts           # PBR material presets
├── shaders/
│   ├── LightningShader.ts          # Lightning effect GLSL
│   └── SqueezeShader.ts            # Baccarat squeeze GLSL
├── effects/
│   ├── CasinoPostProcessing.tsx    # Bloom + tone mapping
│   ├── LightningEffect.tsx         # Lightning mesh component
│   └── SqueezeCard.tsx             # Squeezable card component
├── environments/
│   ├── LightingRig.tsx             # Preset-based lighting
│   └── EnvironmentManager.tsx      # HDR + tone mapping
├── audio/
│   ├── CollisionSound.tsx          # Physics collision audio
│   ├── PositionalAudioEmitter.tsx  # Spatial audio
│   └── AmbientSoundscape.tsx       # Background ambience
├── cards/
│   ├── CardPoolManager.ts          # Object pool for cards
│   ├── CardDealAnimation.ts        # Bezier deal system
│   └── CardPeekAnimation.ts        # Dealer peek animation
└── scenes/
    ├── roulette/
    ├── craps/
    ├── sicbo/
...
| Easy Mode | ✅ (Craps) | ✅ | Simplified betting UI |
| My Numbers | ✅ (Craps) | ✅ | Personalized payout display |
| Mechanical Arm | ✅ (Craps) | ✅ | Animated dice shooter |
| Squeeze Cards | ✅ (Baccarat) | ✅ | Interactive card reveal |
| Shuffle on Demand | ✅ (Baccarat) | ✅ | Player-controlled shoe reset |
| Free Hands | ✅ (Baccarat) | ✅ | Trend building |
| Multi-Table | ✅ (Baccarat, 12 tables) | Phase 2 | Requires chain support |
| Go Live Portal | ✅ | Future | Transition to live dealer |
| Dynamic Lighting | ✅ (Dragon Tiger) | ✅ | Outcome-driven color shifts |
| Fast Play | ✅ | ✅ | Deal Now, skip animations |
| Tutorial Mode | ✅ | ✅ | Tooltips, guided play |
| Roadmaps | ✅ (Baccarat) | ✅ | Big Road, Bead Plate |
| Lightning Multipliers | ✅ (Lightning Roulette) | Phase 2 | Requires chain schema |
| 3D Chip Stacks | ✅ | ✅ | Realistic bet visualization |
| Side Bet Spots | ✅ (Blackjack) | ✅ | Perfect Pairs, 21+3 |
| Table Sorting | ✅ (Baccarat) | ✅ | Sort by streak/pattern |
...
|------|-----------|----------|-------|------------|
| Roulette | ✅ | ✅ | 1 | Medium |
| Lightning Roulette | ✅ | ✅ | 2 | Medium |
| Craps | ✅ | ✅ | 1 | High |
| Blackjack | ✅ | ✅ | 1 | Low |
| Baccarat | ✅ | ✅ | 1 | Medium |
| Dragon Tiger | ✅ | ✅ | 1 | Low |
| Sic Bo | ✅ | ✅ | 1 | Medium |
| Three Card Poker | ✅ | ✅ | 2 | Low |
| Casino Hold'em | ✅ | ✅ | 2 | Medium |
| Ultimate Texas Hold'em | ✅ | ✅ | 2 | Medium |
| HiLo | ✅ | ✅ | 2 | Low |
| Video Poker | ❌ | ✅ | 2 | Low |
| Football Studio | ✅ | ✅ | 3 | Low |
| Mega Ball | ✅ | ✅ | 3 | Very High |
| Dream Catcher | ✅ | ✅ | 3 | High |
| Crazy Time | ✅ | Phase 4 | Future | Extreme |
...
- Emphasize that physics looks random because the *launch* is random—only the *final position* is predetermined (by provably fair RNG)
