...
export const trackGatewayResponse = (session: Session | undefined, response: Record<string, unknown>) => {
  if (!hasOps() || !response) return;
  const type = response.type as string | undefined;
  if (!type) return;
  ...
  if (type === 'game_result') {
    ...
    const startNum = session?.lastGameStartChips !== undefined
      ...
      : undefined;
    const netPnL =
      finalNum !== undefined && startNum !== undefined
        ? finalNum - startNum
        : undefined;
...
export const trackGatewaySession = (session: Session | undefined) => {
  if (!session || !hasOps()) return;
  void sendOpsEvent('casino.session.started', {
    source: 'gateway',
  }, session);
};

export const trackGatewayFaucet = (session: Session | undefined, amount: bigint) => {
  if (!session || !hasOps()) return;
