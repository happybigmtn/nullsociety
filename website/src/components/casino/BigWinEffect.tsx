...
export const BigWinEffect: React.FC<BigWinEffectProps> = ({ amount, show, durationMs, reducedMotion = false }) => {
    ...
    return (
        <div className="absolute inset-0 z-[100] flex items-center justify-center pointer-events-none overflow-hidden">
            ...
            <div className={`absolute inset-0 ${reducedMotion ? 'bg-terminal-green/10' : 'bg-terminal-green/20 animate-pulse'}`}></div>
            ...
            {/* Text Overlay */}
            <div className={`relative flex flex-col items-center gap-2 ${reducedMotion ? '' : 'animate-bounce-in'}`}>
