 ...
 */
export const gameTypeSchema = z.number().int().min(0).max(9).refine(
 ...
 */
export const betAmountSchema = z.string().regex(/^\d+$/, 'Bet must be numeric string').refine(
  (val) => {
    try {
...
export const positiveBetAmountSchema = betAmountSchema.refine(
  (val) => {
    try {
